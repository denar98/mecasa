...
class Custom_model extends CI_Model
{
  ...
  public function getDataProjects($limit, $start, $keyword, $project_status, $project_type)
  {
  ...
  public function getDataProjectsTable($project_detail_status,$project_detail_type)
  {
    ...
    $query = $this->db->select('projects.*,project_details.*,clients.*')
             ...
             ->join("project_details","projects.project_id = project_details.project_id")
             ->join("clients","projects.client_id = clients.client_id");
            if( $project_detail_type != 'All' ){
              $this->db->where('project_details.project_detail_type',$project_detail_type);
              }
            if($project_detail_status=='Progress' || $project_detail_status==null ){
              $this->db->where('project_detail_status != "Done"');            
            }
            else if($project_detail_status=='Done'){
              $this->db->where('project_detail_status = "Done"');            
            }
            $query = $this->db->get();
            return $query;
  ...
  public function getDataProjectsTableRow($project_detail_status,$project_detail_type)
  {
  ...
  public function getDataMecasaHistoryFinancial($limit, $start)
  {
    ...
    ->order_by('mecasa_financials.financial_date','desc')
    ->limit($limit, $start);
    $query = $this->db->get();
  ...
  public function getTotalIncomeByType($type)
  {

    $this->db->select('SUM(project_detail_nominal) as total')
    ->from('projects')
    ->join("project_details","projects.project_id = project_details.project_id")
    ->where('project_detail_type',$type);
    $query = $this->db->get();
  ...
  public function getTotalUangMasukByType($type)
  {

    $this->db->select('SUM(financial_nominal) as total')
    ->from('project_financials')
    ->join('project_details','project_details.project_detail_id = project_financials.project_detail_id')
    // ->where('financial_ops_type','Operasional Project')
    ->where('project_detail_type',$type)
    ->where('financial_type','Uang Masuk');
    $query = $this->db->get();
  ...
  public function getTotalUangKeluarByType($type)
  {
  ...
  public function getFinancialMecasaTotalMonthly($financial_type)
  {

    $query = $this->db->query("SELECT 
    SUM(IF(month = 'Jan', financial_nominal, 0)) AS 'Jan',
    SUM(IF(month = 'Feb', financial_nominal, 0)) AS 'Feb',
    SUM(IF(month = 'Mar', financial_nominal, 0)) AS 'Mar',
    SUM(IF(month = 'Apr', financial_nominal, 0)) AS 'Apr',
    SUM(IF(month = 'May', financial_nominal, 0)) AS 'May',
    SUM(IF(month = 'Jun', financial_nominal, 0)) AS 'Jun',
    SUM(IF(month = 'Jul', financial_nominal, 0)) AS 'Jul',
    SUM(IF(month = 'Aug', financial_nominal, 0)) AS 'Aug',
    SUM(IF(month = 'Sep', financial_nominal, 0)) AS 'Sep',
    SUM(IF(month = 'Oct', financial_nominal, 0)) AS 'Oct',
    SUM(IF(month = 'Nov', financial_nominal, 0)) AS 'Nov',
    SUM(IF(month = 'Dec', financial_nominal, 0)) AS 'Dec'
    FROM
    (SELECT MIN(DATE_FORMAT(financial_date, '%b')) AS month, SUM(financial_nominal) as financial_nominal
     FROM mecasa_financials WHERE financial_type = '$financial_type'
     group by YEAR(financial_date), MONTH(financial_date) order by  YEAR(financial_date), MONTH(financial_date)) as sale");
