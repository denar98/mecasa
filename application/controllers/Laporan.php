...
class Laporan extends CI_Controller {
	...
	public function keuangan()
	{
		...
		$data['mecasa_masuk_monthly'] = $this->custom_model->getFinancialMecasaTotalMonthly('Uang Masuk')->result();
		$data['mecasa_keluar_monthly'] = $this->custom_model->getFinancialMecasaTotalMonthly('Uang Keluar')->result();
		// print_r($data['mecasa_keluar_monthly']);
		// exit();
		...
		$income_produksi  = $this->custom_model->getTotalIncomeByType('Produksi')->row();
		$income_design  = $this->custom_model->getTotalIncomeByType('Design')->row();
		$total_uang_masuk_produksi  = $this->custom_model->getTotalUangMasukByType('Produksi')->row();
		$total_uang_keluar_produksi  = $this->custom_model->getTotalUangKeluarByType('Produksi')->row();
		$total_uang_masuk_design  = $this->custom_model->getTotalUangMasukByType('Design')->row();
		$total_uang_keluar_design  = $this->custom_model->getTotalUangKeluarByType('Design')->row();
		$data['total_uang_sisa_design']  = $income_design->total - $total_uang_masuk_design->total;
		$data['total_uang_sisa_produksi']  = $income_produksi->total - $total_uang_masuk_produksi->total;
		
		$data['total_uang_masuk_design']  = $total_uang_masuk_design->total - $total_uang_keluar_design->total;
		$data['total_uang_masuk_produksi']  = $total_uang_masuk_produksi->total - $total_uang_keluar_produksi->total;
	...
	public function addHistoryAction()
  {
    ...
    $financial_out_type = $this->input->post('financial_out_type');
	...
	'financial_out_type' => $financial_out_type,
     ...
     foreach($data->result() as $financial)
     {
		...
		$output .= '
			...
			<li class="list-group-item" style="margin-top:-31px; margin-left:10px;">
				<a  data-toggle="modal" data-target="#editHistoryModal" style="cursor: pointer;" onclick="updateHistoryFinancial('.$financial->mecasa_financial_id.')"><p style="font-weight: bold; margin-bottom:0px;"> Rp. '.number_format($financial->financial_nominal).' - <span style="font-weight: 300;">'.$financial->financial_type.'</span> - <span style="font-weight: 300;">'.$financial->financial_out_type.'</span>  </p></a>
