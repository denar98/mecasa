        ...
        foreach($tags->result() as $rows)
        {
          ...
          $i++;
            $data[]= array(
                $i,
                $rows->tag_name,
                '<a href="#" class="btn btn-warning mr-1 btn-action" style="padding-left:15px; padding-right:15px;" onclick="getTag('.$tag_id.')" data-toggle="modal" data-target="#updateModal"><i class="fa fa-pencil"></i></a>
                 <a href="'.base_url().'Tag/deleteAction/'.$rows->tag_id.'" style="padding-left:15px; padding-right:15px;" class="btn btn-danger mr-1 btn-action"><i class="fa fa-trash"></i></a>'
            );
        }
